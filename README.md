...
A aula tem duas abas de seis atividades: mostrar mensagens com `print` e guardar informações em variáveis. Cada atividade contém uma explicação curta, um exemplo com seu resultado e um exercício executável. As tarefas usam situações variadas e humor, com alteração, preenchimento, correção de erros propositais e criação independente. Dicas graduais ajudam durante a escrita do código. Não há campos de resposta discursiva. A segunda aba pode ser trabalhada em outro encontro.
...
A aula usa chaves próprias para salvar os códigos. O armazenamento é local ao navegador, sem envio a um servidor. Os registros de versões anteriores permanecem intactos em suas chaves antigas.
